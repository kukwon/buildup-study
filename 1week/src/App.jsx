...
function useSingularForm() {
  ...
  function validate(input) {
    ...
    if (!changed.maximum || !changed.minimum || !changed.required)
      ref.current?.focus();
    setValid(changed);

    const error = {};
    if (valid.maximum === false)
      error.maximum = "비밀번호는 10글자를 넘을 수 없습니다.";
    if (valid.minimum === false)
      error.minimum = "비밀번호는 5글자를 넘어야 합니다.";
